...
const OAuth2Login = ({ onUserChange }) => {
  ...
  useEffect(() => {
    ...
    fetch("https://auth.famquest.nephele.ari-imet.eu/oauth2/userinfo", { credentials: "include" })
      .then((res) => {
  ...
  const login = () => {
    ...
    const loginUrl = `https://auth.famquest.nephele.ari-imet.eu/oauth2/sign_in?rd=${encodeURIComponent(frontendUrl)}`;
